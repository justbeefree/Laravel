                            ...
                            <form action="{{route('admin.order.update', $order)}}" method="post">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control" id="inputName" name="name"
                                           aria-describedby="{{__('Имя заказчика')}}" value="{{old("name")?:$order->name}}">
                                </div>
                                <div class="mb-3">
                                    <label for="inputPhone" class="form-label required">{{__("Телефон")}}</label>
                                    <input type="text" name="phone" class="form-control" id="inputPhone" placeholder="+7 (XXX) XXX-XX-XX" value="{{old("phone")?:$order->phone}}">
                                </div>
                                <div class="mb-3">
                                    <label for="inputEmail" class="form-label required">{{__("Email")}}</label>
                                    <input type="email" name="email" class="form-control" id="inputEmail" placeholder="name@example.com" value="{{old("email")?:$order->email}}">
                                </div>
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" id="detailText" name="textInfo"
                                              rows="3">{{old("textInfo")?:$order->text}}</textarea>
                                </div>
